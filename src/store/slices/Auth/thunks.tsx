...
import { StorageKeys, deleteItemFromStorage, getItemFromStorage, setItemToStorage } from "../../../utils/storage";
import { AppDispatch, GetState } from "../../store";
import { authSuccess, getAuthFromStorageSuccess, resetAuthStateSuccess } from "./reducers";
  ...
  export const resetAuthState = async (dispatch: AppDispatch) => {
    try {
      await Promise.all([
        deleteItemFromStorage(StorageKeys.ACCESS_TOKEN),
      ]);
  ...
  export const fetchLogin =
    ...
    async (dispatch: AppDispatch, getState: GetState) => {
      try {
        ...
        const res = await fetchLoginApi({client_id,client_secret,grant_type});
  console.log(res)
        if (!res) return;
        await Promise.all([
